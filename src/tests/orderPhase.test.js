...
//     const scoops = screen.getByText('Scoops: $', {exact: false})
//     const toppings = screen.getByText('Toppings: $', {exact: false})
...
//
//     // confirm order number on confirmation page, async because there is a post
...
//     const newOrder = screen.getByRole('button', {name:'Create new order'})
//     userEvent.click(newOrder)
//
//     //check that scoops and toppings subtotals have been reset
//     expect(scoops).toHaveTextContent('0.00')
//     expect(toppings).toHaveTextContent('0.00')
//
...
test('Toppings header is not on summary page if no toppings ordered', async()=>{
  ...
  const scoopsHeading = screen.getByRole('heading', {name: 'Scoops: $6.00'})
  expect(scoopsHeading).toBeInTheDocument();
  // querybyrole cuando no queremos que este en la pagina
